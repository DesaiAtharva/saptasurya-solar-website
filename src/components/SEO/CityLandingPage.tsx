...
import {
  Box, Container, Typography, Grid, Card, CardContent, Button, Chip, List, ListItem,
  ListItemIcon, ListItemText, useTheme as useMuiTheme, Link,
} from '@mui/material';
import {
  CheckCircle, WbSunny, Bolt, Savings, Phone, WhatsApp, LocationOn, Star, EmojiEvents,
...
export default function CityLandingPage({ citySlug, cityName }: CityLandingPageProps) {
  ...
  const services = [
    { icon: <WbSunny sx={{ color: theme.palette.primary.main, fontSize: 36 }} />, title: 'Rooftop Solar', desc: 'Complete rooftop solar system for homes and businesses.' },
    { icon: <Bolt sx={{ color: theme.palette.primary.main, fontSize: 36 }} />, title: 'Solar Water Heater', desc: 'Save 70% on water heating costs with solar technology.' },
    { icon: <Savings sx={{ color: theme.palette.primary.main, fontSize: 36 }} />, title: 'Solar Pump', desc: 'Government-subsidised solar pumps for agriculture.' },
    { icon: <EmojiEvents sx={{ color: theme.palette.primary.main, fontSize: 36 }} />, title: 'EPC Services', desc: 'End-to-end Engineering, Procurement & Construction.' },
  ];
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Solar Installation', item: 'https://saptasuryasolar.com/services' },
      { '@type': 'ListItem', position: 3, name: cityName, item: `https://saptasuryasolar.com/solar-panel-installation-${citySlug}` },
    ],
  };
  ...
  return (
    <Box>
      ...
      {/* Hero Section */}
      <Box sx={{
        minHeight: '60vh', display: 'flex', alignItems: 'center', position: 'relative', overflow: 'hidden',
        ...
        <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1, py: 10 }}>
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Chip label={`Serving ${cityName}, Maharashtra`} color="primary" sx={{ mb: 3, fontWeight: 600 }} />
            <Typography variant="h1" sx={{
              fontWeight: 800, mb: 3, lineHeight: 1.15,
              background: `linear-gradient(135deg, ${theme.palette.text.primary} 0%, ${theme.palette.primary.main} 100%)`,
              WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
            }}>
              Solar Panel Installation<br />in {cityName}
            </Typography>
            <Typography variant="h5" color="text.secondary" sx={{ mb: 4, fontWeight: 400, lineHeight: 1.6, maxWidth: 650 }}>
              Maharashtra's trusted solar energy company. Get affordable rooftop solar systems with government subsidy in {cityName}.
            </Typography>
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
              <Button
                ...
                target="_blank" sx={{ px: 4, py: 1.5, fontSize: '1.1rem' }}
              >
                Get Free Quote
              </Button>
              <Button
                variant="outlined" size="large" startIcon={<Phone />}
                href="tel:+919860874908" sx={{ px: 4, py: 1.5, fontSize: '1.1rem' }}
              >
                Call Now
              </Button>
            </Box>
          ...
          <Grid container spacing={2} justifyContent="center">
            {stats.map((s) => (
              <Grid item xs={6} md={3} key={s.label} sx={{ textAlign: 'center' }}>
                <Typography variant="h4" sx={{ fontWeight: 800, color: 'primary.contrastText' }}>{s.number}</Typography>
                <Typography variant="body2" sx={{ color: 'rgba(0,0,0,0.7)', fontWeight: 600 }}>{s.label}</Typography>
              </Grid>
            ))}
          </Grid>
        </Container>
        ...
        <Container maxWidth="lg">
          <Grid container spacing={6} alignItems="center">
            <Grid item xs={12} md={6}>
              <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                ...
                <Typography color="text.secondary" sx={{ mb: 4, lineHeight: 1.8 }}>
                  With years of experience serving {cityName} and surrounding districts, we bring expertise, quality, and transparent pricing to every solar installation.
                </Typography>
                <List disablePadding>
                  {benefits.map((b) => (
            ...
            <Grid item xs={12} md={6}>
              <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                <Box sx={{
                  borderRadius: 4, overflow: 'hidden', border: `2px solid ${theme.palette.primary.main}`,
      ...
      {/* Services Section */}
      <Box sx={{ py: 8, bgcolor: 'background.paper', borderTop: `1px solid ${theme.palette.divider}` }}>
        <Container maxWidth="lg">
          <Typography variant="h2" sx={{ fontWeight: 800, mb: 2, textAlign: 'center' }}>Our Solar Services in {cityName}</Typography>
          <Typography color="text.secondary" sx={{ textAlign: 'center', mb: 6, maxWidth: 550, mx: 'auto' }}>
      ...
      <Box sx={{
        ...
        <Container maxWidth="md">
          ...
          <Typography sx={{ color: 'rgba(0,0,0,0.75)', mb: 4, fontSize: '1.1rem' }}>
            Our expert team will analyse your roof, calculate savings, and provide a transparent proposal — completely free.
          </Typography>
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button
              ...
              target="_blank"
              sx={{ bgcolor: '#ffffff', color: theme.palette.primary.dark, px: 4, py: 1.5, fontWeight: 700, '&:hover': { bgcolor: '#f0f0f0' } }}
            >
              WhatsApp Us
            </Button>
            <Button
              variant="outlined" size="large" startIcon={<LocationOn />}
              href="/contact"
              sx={{ borderColor: 'white', color: 'white', px: 4, py: 1.5, fontWeight: 700, '&:hover': { borderColor: 'white', bgcolor: 'rgba(255,255,255,0.1)' } }}
            >
              ...
              .map((c) => (
                <Link
                  ...
                  sx={{
                    ...
                    color: 'text.secondary', fontSize: '0.9rem', fontWeight: 500,
                    '&:hover': { color: 'primary.main', borderColor: 'primary.main', bgcolor: `${theme.palette.primary.main}08` },
